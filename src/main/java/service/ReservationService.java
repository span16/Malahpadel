...
public class ReservationService implements Iservice<reservation> {
    ...
    public void ajouter(reservation r) throws SQLException {
        String sql = "INSERT INTO reservation (nombre_places, type_reservation, code_confirmation, remarque) VALUES (?, ?, ?, ?)";
        ...
        Evenement evenement = r.getEvenement();
        if (evenement != null) {
            EvenementService evenementService = new EvenementService();
            Evenement evenementRecupere = evenementService.getEvenementByNom(evenement.getNom());

            if (evenementRecupere != null) {
                // Associe l'événement récupéré à la réservation
                r.setEvenement(evenementRecupere);
            } else {
                throw new SQLException("L'événement avec ce nom n'a pas été trouvé.");
            }
        } else {
            throw new SQLException("L'événement associé à la réservation est introuvable.");
        }
    ...
    public List<reservation> recuperer() throws SQLException {
        String sql = "SELECT r.id_R, r.nombre_places, r.type_reservation, r.code_confirmation, r.remarque " +
                "FROM reservation r";
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(sql);
        List<reservation> reservations = new ArrayList<>();

        EvenementService evenementService = new EvenementService();  // Service pour récupérer les événements

        while (rs.next()) {
            // Récupère l'événement par son nom (si nécessaire)
            Evenement evenement = evenementService.getEvenementByNom("Nom de l'événement"); // Remplacez par le nom réel
    ...
    public List<reservation> filterByCodeConfirmation(String code) throws SQLException {
        String sql = "SELECT r.id_R, r.nombre_places, r.type_reservation, r.code_confirmation, r.remarque, e.nom AS evenement_nom " +
                "FROM reservation r " +
                "LEFT JOIN evenement e ON r.id_evenement = e.id " +
        ...
        while (rs.next()) {
            ...
            evenement.setNom(rs.getString("evenement_nom"));
